...
public class SkippableEdge {

    List<DirectedEdge> result;

    public SkippableEdge(EdgeWeightedDigraph g, int s, int t) {
        DijkstraSP sp = new DijkstraSP(g, s);
        Iterable<DirectedEdge> iterS = sp.pathTo(t);
        List<DirectedEdge> list = new LinkedList<>();
        for (edu.princeton.cs.algs4.DirectedEdge e : iterS) {
            list.add(e);
        }
        ...
        for (int v = g.V() - 1; v >= 0; v--) {
            for (edu.princeton.cs.algs4.DirectedEdge w : g.adj(v)) {
                reverse.addEdge(w);
            }
        }
    ...
    public static void main(String[] args) {
        ...
        // compute shortest paths
        SkippableEdge sp2 = new SkippableEdge(G, s, t);
