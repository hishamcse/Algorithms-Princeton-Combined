...
public class FastCollinearPoints {
    ...
    public FastCollinearPoints(
        ...
        for (int i = 0; i < copy.length; i++) {
            ...
            //Mark(94/100)
           // for (int j = 0; j < all.length; d = 1) {
           //
           //     do {
           //         d++;
           //         j++;
           //     } while (j < all.length && copy[i].slopeTo(all[j]) == copy[i].slopeTo(all[j - 1]));
           //
           //     if (d >= 4 && !list.contains(all[j - 1])) {
           //         if (!list.contains(copy[i])) {
           //             arr.add(new LineSegment(copy[i], all[j - 1]));
           //         } else {
           //             if (copy[i].compareTo(all[j - 1]) <= 0) {
           //                 arr.add(new LineSegment(copy[i], all[j - 1]));
           //             }
           //         }
           //         list.add(all[j - 1]);
